...
def venues():
	...
	# To ignore duplicate venues we need to use set
	# initialise locations as set()
    locations = set()
  ...
  # add venues for each city and state 
    ...
    for venue in cities:
        ...
        # Filter no_upcoming_shows to show current data
...
def show_artist(artist_id):
    ...
    for show_all in shows:
        data = {
            "venue_id": show_all.venue_id,
            "venue_name": show_all.venue.name,
            "venue_image_link": show_all.venue.image_link,
            "start_time": format_datetime(str(show_all.start_time))
        }
        if show_all.start_time > current_time:
...
def edit_artist_submission(artist_id):
  ...
  try:
        ...
        db.session.rolback()
        flash('An Error has occurred and the update unsuccessful')
  finally:
        db.session.close()
...
def delete_artist(artist_id):
    try:
        ...
        # delete artist from database and commit
